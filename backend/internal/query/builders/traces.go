...
func BuildTracesQuery(filters map[string]string, from, to time.Time, limit, offset int) string {
	base := "SELECT TraceId AS traceId, argMin(SpanName, Timestamp) AS name, any(ServiceName) AS serviceName, count() AS spanCount, max(Timestamp) AS lastSeen, dateDiff('millisecond', min(Timestamp), max(addNanoseconds(Timestamp, Duration))) AS durationMs FROM telemetry.otel_traces"
