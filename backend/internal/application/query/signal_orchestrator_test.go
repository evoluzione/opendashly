...
func TestSignalOrchestrator_RunSuccessWithPaginationAndCursor(t *testing.T) {
	...
	result := orchestrator.run(context.Background(), nil, signalQueries{}, map[string]bool{"logs": true, "traces": true}, 1)
...
func TestSignalOrchestrator_RunCollectsPartialErrors(t *testing.T) {
	...
	result := orchestrator.run(context.Background(), nil, signalQueries{}, map[string]bool{"logs": true, "traces": true}, 100)
...
func TestSignalOrchestrator_RunNilOrchestratorReturnsEmptyResult(t *testing.T) {
	...
	if len(result.logs) != 0 || len(result.traces) != 0 {
